...
usage() {
  cat <<'EOF'
Usage:
  scripts/build/build-srdf-skill.sh [--check] [--clean]

Vendors packages/cadpy_metadata into skills/srdf/scripts/packages/cadpy_metadata.
...
if ! command -v rsync >/dev/null 2>&1; then
  echo "rsync is required to vendor cadpy_metadata into the SRDF skill runtime." >&2
  exit 1
fi
...
check_runtime() {
  ...
  local diff_path="${TMPDIR:-/tmp}/srdf-skill-cadpy-metadata-diff.txt"
  if [ ! -d "$RUNTIME_DIR" ]; then
    echo "Missing generated cadpy_metadata runtime: $label" >&2
    echo "Run scripts/build/build-srdf-skill.sh and commit the generated copy." >&2
    exit 1
  fi
  if ! diff -qr \
    -x __pycache__ \
...
if [ "$MODE" = "check" ]; then
  ...
  sync_runtime
  echo "Built ${RUNTIME_DIR#$REPO_ROOT/}"
fi
