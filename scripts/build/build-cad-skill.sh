...
usage() {
  cat <<'EOF'
Usage:
  scripts/build/build-cad-skill.sh [--check] [--clean] [--no-install]

Builds the self-contained browser runtime used by skills/cad/scripts/cadpy_snapshot
and the bundled Python package runtime used by skills/cad/scripts.

Options:
  --check       Build into tmp/ and fail if snapshot/runtime is stale.
...
check_runtime() {
  ...
  if [ "$stale" -ne 0 ]; then
    echo "" >&2
    echo "Run scripts/build/build-cad-skill.sh and commit the updated runtime files." >&2
    exit 1
  fi
...
check_cadpy_runtime() {
  ...
  if ! diff -qr \
    ...
    echo "Run scripts/build/build-cad-skill.sh and commit skills/cad/scripts/packages/cadpy." >&2
    exit 1
  fi
...
if [ "$MODE" = "check" ]; then
  ...
  echo "Built skills/cad/scripts/cadpy_snapshot/runtime"
  echo "Built skills/cad/scripts/packages/cadpy"
fi
