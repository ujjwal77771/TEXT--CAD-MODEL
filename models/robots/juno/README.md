...
- `.juno.step.js` — CAD Viewer animation sidecar with four in-place gaits
  ...
  arm swing, torso counter-sway, and head stabilization. Controls:
  `phase`, `gait`, `strideLength`, `legLift`, `armSwing`, `torsoSway`.
  Occurrence refs `#o1.1..#o1.28` follow the `asm.add` order in `juno.py`.
