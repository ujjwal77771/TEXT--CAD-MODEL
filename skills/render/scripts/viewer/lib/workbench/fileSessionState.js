...
export const FILE_SESSION_STORAGE_KEY_PREFIX = "cad-explorer:file-session";
const FILE_SESSION_INDEX_KEY_PREFIX = "cad-explorer:file-session:index";
...
function entryAssetHash(entry, key) {
  return normalizeString(entry?.assets?.[key]?.hash);
}
...
function entryStepModuleSignature(entry) {
  return [
    ...
    normalizeString(entry?.step?.hash)
  ].filter(Boolean).join(":");
}
...
function normalizeThemeSlice(value) {
  if (!isPlainObject(value)) {
    return null;
  }
  const settings = isPlainObject(value.settings) ? normalizeThemeSettings(value.settings) : null;
  if (!settings) {
    return null;
  }
  return {
    presetId: normalizeString(value.presetId),
    settings
  };
}
